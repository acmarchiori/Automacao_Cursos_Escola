...
def converter_imagens_para_base64(html_path, image_folder_path):
    """
    Converte todas as imagens em um arquivo HTML para base64 e substitui as referências
    por dados base64 no próprio HTML.
    """
    ...
    for img_tag in soup.find_all('img'):
        ...
        img_file_path = os.path.join(image_folder_path, os.path.basename(img_src))
...
def processar_arquivos_zip(pasta_zip):
    """
    Processa todos os arquivos .zip em uma pasta, converte as imagens para base64
    nos arquivos HTML descompactados e salva o HTML atualizado.
    """
                ...
                for sub_root, sub_dirs, sub_files in os.walk(temp_folder):
                    for sub_file in sub_files:
                        if sub_file.endswith('.html'):
                            ...
                            image_folder = sub_root  # Assumimos que todas as imagens estão aqui
                
                if html_file and image_folder:
                    print(f"Processando HTML: {html_file}")
                    # Converte imagens para base64 e salva o HTML atualizado
                    html_content_com_base64 = converter_imagens_para_base64(html_file, image_folder)
                    
                    # Sobrescreve o HTML original com o conteúdo atualizado
                    with open(html_file, 'w', encoding='utf-8') as file:
                        file.write(html_content_com_base64)
                    print(f"HTML atualizado com imagens em base64 salvo em {html_file}.")
                else:
                    print(f"Erro: Arquivo HTML ou pasta de imagens não encontrado em {temp_folder}")
